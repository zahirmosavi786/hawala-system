...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_final'])) {
    ...
    $hawala_date    = $_POST['hawala_date'] ?? $created_at;
    ...
    if(empty($missing)){
        ...
        if($query_hawala->insert_hawala(
            ...
            $_SESSION['success_message'] = "حواله با موفقیت ثبت شد!";
            header("Location: ".$_SERVER['PHP_SELF']);
            exit;
        }else{
            $_SESSION['danger_message'] = "خطا در ثبت حواله!";
        }
    }
}
?>
...
<section class="content-header">
    <div class="container-fluid">
        <h1 class="text-info"><i class="fa fa-exchange-alt"></i> ثبت حواله جدید</h1>
    </div>
</section>
...
<div class="card-header bg-info text-white">
    <h3 class="card-title"><i class="fa fa-hand-holding-usd"></i> فرم ثبت حواله</h3>
</div>

<form id="hawalaForm"  action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
<div class="card-body">
...
<div class="row g-3 mt-2">
    ...
    <div class="col-md-4">
        ...
        <input type="text" name="hawala_no" class="form-control form-control-sm" 
               ...
               value="<?= htmlspecialchars($_POST['hawala_no'] ?? '', ENT_QUOTES) ?>">
    ...
    <div class="col-md-4">
        ...
        <input type="text" name="from_name" class="form-control form-control-sm" 
               placeholder="نام فرستنده" value="<?= htmlspecialchars($_POST['from_name'] ?? 'سیدنبی', ENT_QUOTES) ?>" readonly>
    ...
    <div class="col-md-4">
        ...
        <input type="text" name="receiver_name" class="form-control form-control-sm" 
               ...
               value="<?= htmlspecialchars($_POST['receiver_name'] ?? '', ENT_QUOTES) ?>">
...
<div class="row g-3 mt-2">
    ...
    <div class="col-md-4">
        ...
        <input type="number" name="amount_hawala" id="amount_hawala" class="form-control form-control-sm" 
               ...
               value="<?= htmlspecialchars($_POST['amount_hawala'] ?? '', ENT_QUOTES) ?>">
    ...
    <div class="col-md-4">
        ...
        <select name="currency" id="currency" class="form-control select2 form-control-sm">
           ...
           <option value="AFN" <?= (($_POST['currency'] ?? '')=='AFN') ? 'selected' : '' ?>>افغانی</option>
           <option value="PKR" <?= (($_POST['currency'] ?? '')=='PKR') ? 'selected' : '' ?>>کلدار</option>
           <option value="USD" <?= (($_POST['currency'] ?? '')=='USD') ? 'selected' : '' ?>>دلار</option>
           <option value="EUR" <?= (($_POST['currency'] ?? '')=='EUR') ? 'selected' : '' ?>>یورو</option>
           <option value="IQD" <?= (($_POST['currency'] ?? '')=='IQD') ? 'selected' : '' ?>>دینار</option>
           <option value="IRR" <?= (($_POST['currency'] ?? '')=='IRR') ? 'selected' : '' ?>>تومان</option>
        </select>
    ...
    <div class="col-md-4">
        ...
        <select name="status" class="form-control form-control-sm">
            <option value="تحویل شده" <?= (($_POST['status'] ?? '')=='تحویل شده')?'selected':'' ?>>تحویل شده</option>
            <option value="کنسل شده" <?= (($_POST['status'] ?? '')=='کنسل شده')?'selected':'' ?>>کنسل شده</option>
            <option value="در انتظار" <?= (($_POST['status'] ?? '')=='در انتظار')?'selected':'' ?>>در انتظار</option>
        </select>
    </div>
...
<div class="col-md-4">
    ...
    <input type="datetime-local" 
           ...
           id="hawala_date" 
           class="form-control"
           value="<?= htmlspecialchars($_POST['hawala_date'] ?? '', ENT_QUOTES) ?>">
    ...
    <div class="col-md-4">
        <label>آدرس مبدا حواله</label>
         <select name="from_address" id="from_address" class="form-control select2 form-control-sm">
           <option value="ایران، قم" <?= (($_POST['from_address'] ?? '')=='ایران، قم') ? 'selected' : '' ?>>ایران، قم</option>
           <option value="عراق، نجف" <?= (($_POST['from_address'] ?? '')=='عراق، نجف') ? 'selected' : '' ?>>عراق، نجف</option>
        </select>
    ...
    <div class="col-md-4">
        ...
        <option value="" disabled <?= !isset($_POST['to_address']) ? 'selected' : '' ?>>-- انتخاب مقصد حواله --</option>
        <option value="افغانستان، سنگماشه" <?= (($_POST['to_address'] ?? '')=='افغانستان، سنگماشه') ? 'selected' : '' ?>>افغانستان، سنگماشه</option>
        <option value="افغانستان، انگوری" <?= (($_POST['to_address'] ?? '')=='افغانستان، انگوری') ? 'selected' : '' ?>>افغانستان، انگوری</option>
        </select>
...
<div class="row g-3 mt-2">
    ...
    <div class="col-md-4">
        ...
        <select name="sender_id" class="form-control select2">
            ...
            <option value="<?= $c['id'] ?>" <?= (($_POST['sender_id'] ?? '')==$c['id'])?'selected':'' ?>>
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="0.01" name="rate_customer" id="rate_customer" class="form-control form-control-sm" 
               ...
               value="<?= htmlspecialchars($_POST['rate_customer'] ?? '', ENT_QUOTES) ?>">
...
<div class="row g-3 mt-2">
    ...
    <div class="col-md-4">
        ...
        <select name="seller_id" class="form-control select2">
            ...
            <option value="<?= $s['id'] ?>" <?= (($_POST['seller_id'] ?? '')==$s['id'])?'selected':'' ?>>
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="0.01" name="rate_seller" id="rate_seller" class="form-control form-control-sm" 
               ...
               value="<?= htmlspecialchars($_POST['rate_seller'] ?? '', ENT_QUOTES) ?>">
