...
$seller = $query_seller->get_seller_by_id($id);
if (!$seller) {
    echo "<div class='alert alert-danger text-center m-3'>فروشنده مورد نظر یافت نشد!</div>";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // بررسی فیلدهای ضروری
    $required = ['name', 'last_name', 'book', 'phone'];
    ...
    if (empty($missing)) {
        // ویرایش در دیتابیس
        if ($query_seller->update_seller(
            $id, $name, $last_name, $father_name, $phone,
            $introducer, $book, $address, $description
        )) {
             session_start();
            $_SESSION['success_message'] =  "اطلاعات فروشنده با موفقیت ویرایش شد!";
            header("Location: edit_seller.php?id=$id&success=1");
            exit();
        } else {
             session_start();
...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2 align-items-center">
      <div class="col-sm-6">
        <h1 class="text-primary"><i class="fa fa-edit"></i> ویرایش فروشنده</h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-left">
          <li class="breadcrumb-item"><a href="#"><i class="fa fa-home"></i> خانه</a></li>
          <li class="breadcrumb-item active"><i class="fa fa-user"></i> ویرایش فروشنده</li>
        </ol>
      </div>
    </div>
  </div>
      ...
      <div class="col-md-10">
        <div class="card shadow-lg border-0 rounded-3">
          <div class="card-header bg-primary text-white">
            <h3 class="card-title"><i class="fa fa-user-edit"></i> اطلاعات فروشنده</h3>
          </div>
          ...
          <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) . '?id=' . urlencode($id); ?>" method="post">
              ...
              <div class="row g-3">
                <div class="col-md-4">
                  ...
                  <input type="text" name="name" class="form-control" placeholder = "اسم فروشنده را وارد کنید"
                         value="<?= htmlspecialchars($_POST['name'] ?? $seller['name'], ENT_QUOTES) ?>">
                ...
                <div class="col-md-4">
                  ...
                  <input type="text" name="last_name" class="form-control" placeholder = "تخلص فروشنده را وارد کنید"
                         value="<?= htmlspecialchars($_POST['last_name'] ?? $seller['last_name'], ENT_QUOTES) ?>">
                  <?php if (in_array('last_name', $missing)): ?><span class="text-danger small">ضروری</span><?php endif; ?>
                </div>

                <div class="col-md-4">
                  <label>اسم پدر</label>
                  <input type="text" name="father_name" class="form-control" placeholder = "اسم پدر فروشنده را وارد کنید"
                         value="<?= htmlspecialchars($_POST['father_name'] ?? $seller['father_name'], ENT_QUOTES) ?>">
                </div>
              </div>
              ...
              <div class="row g-3 mt-2">
                <div class="col-md-4">
                  ...
                  <input type="text" name="phone" class="form-control" placeholder = "مثلاً 0935xxxxxxx"
                         value="<?= htmlspecialchars($_POST['phone'] ?? $seller['phone'], ENT_QUOTES) ?>">
                </div>

                <div class="col-md-4">
                  <label>معرف</label>
                  <input type="text" name="introducer" class="form-control" placeholder="نام معرف (اختیاری)"
                         value="<?= htmlspecialchars($_POST['introducer'] ?? $seller['introducer'], ENT_QUOTES) ?>">
                </div>

                <div class="col-md-4">
                  <label>دفتر مربوطه</label>
                 <select name="book" class="form-control select">
              ...
              <div class="row g-3 mt-2">
                <div class="col-md-12">
                  ...
                  <input type="text" name="address" class="form-control" placeholder="آدرس فروشنده (اختیاری)"
                         value="<?= htmlspecialchars($_POST['address'] ?? $seller['address'], ENT_QUOTES) ?>">
                </div>
              </div>

              <div class="form-group mt-2">
                <label>توضیحات</label>
                <textarea name="description" class="form-control"  placeholder="توضیحات تکمیلی (اختیاری)" rows="2"><?= htmlspecialchars($_POST['description'] ?? $seller['description'], ENT_QUOTES) ?></textarea>
              </div>
            ...
            <div class="card-footer text-center">
              ...
              <a href="list_seller.php" class="btn btn-secondary px-4">بازگشت</a>
            </div>
          </form>
        </div>
      </div>
